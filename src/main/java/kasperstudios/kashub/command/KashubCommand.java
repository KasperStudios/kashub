...
public class KashubCommand {
        ...
        dispatcher.register(
                CommandManager.literal("script")
                        ...
                        .then(CommandManager.literal("stop")
                                .executes(KashubCommand::stopAll))
                        .then(CommandManager.literal("list")
                                .executes(KashubCommand::listScripts))
                        .then(CommandManager.literal("tasks")
                                .executes(KashubCommand::listTasks))
    ...
    private static int runScript(CommandContext<ServerCommandSource> ctx, String name) {
        try {
            ...
            TaskManager.getInstance().startScript(name, content, null, kasperstudios.kashub.core.Type.USER);
    ...
    private static int listTasks(CommandContext<ServerCommandSource> ctx) {
        Collection<kasperstudios.kashub.core.Task> tasks = TaskManager.getInstance().getAllTasks();
        ctx.getSource().sendFeedback(() -> Text.literal("=== Tasks ==="), false);
        for (kasperstudios.kashub.core.Task task : tasks) {
    ...
    private static int pauseTask(CommandContext<ServerCommandSource> ctx, int id) {
        kasperstudios.kashub.core.Task task = TaskManager.getInstance().getTask(id);
        if (task == null) {
