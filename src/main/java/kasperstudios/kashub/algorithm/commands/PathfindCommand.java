...
public class PathfindCommand implements Command {
    ...
    private static Set<net.minecraft.block.Block> dangerousBlocks = null;

    private static Set<net.minecraft.block.Block> getDangerousBlocks() {
    ...
    private void handleConfig(String[] args, ScriptInterpreter interpreter) {
        ...
        switch (option) {
            ...
            case "maxfall":
                try {
                    maxFallDistance = Integer.parseInt(value);
        ...
        for (int i = startIndex; i < args.length; i++) {
            ...
            if (arg.contains("=")) {
                ...
                switch (parts[0]) {
                    ...
                    case "maxfall":
                        try {
                            options.maxFallDistance = Integer.parseInt(parts[1]);
                        } catch (NumberFormatException ignored) {
                        }
                        break;
                    case "sprint":
    ...
    private void navigateTo(ClientPlayerEntity player, BlockPos target, ScriptInterpreter interpreter,
            PathOptions options) {
    ...
    public static void tick() {
        if (!isPathfinding || currentPath == null)
            return;
