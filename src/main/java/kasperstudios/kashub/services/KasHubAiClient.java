...
public class KasHubAiClient {
    ...
    private static final long MODELS_CACHE_TTL = 3600000;
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                throw new IOException(
                        "AI request failed: " + response.code() + " " + response.message() + "\n" + errorBody);
            }
    ...
    private String toolListScripts() {
        try {
            ...
            java.nio.file.Files.list(scriptsDir)
                    ...
                    .forEach(p -> result.append("- ").append(p.getFileName().toString().replace(".kh", ""))
                            .append("\n"));
