package kasperstudios.kashub.server.script.core;
...
public class ExpressionParser {
    ...
    public static class Value {
        ...
        public boolean toBoolean() {
            ...
            if (type == Type.STRING) {
                ...
                return !str.isEmpty() && !str.equals("0") && !str.equalsIgnoreCase("false")
    ...
    // === New KS 2.0 Overloads ===

    /**
     * Evaluate expression with ScriptContext (KS 2.0)
     */
    public static kasperstudios.kashub.server.script.objects.ScriptValue evaluate(
            String expression,
            kasperstudios.kashub.server.script.v2.ScriptContext ctx) {

        // Adapter: Convert ScriptContext to Function<String, String>
        Function<String, String> resolver = varName -> {
            kasperstudios.kashub.server.script.objects.ScriptValue val = ctx.getVariable(varName);
            return val.toString();
        };

        Value result = evaluate(expression, resolver);

        // Convert Value to ScriptValue
        switch (result.getType()) {
            case NUMBER:
                return new kasperstudios.kashub.server.script.objects.ScriptValue(result.toNumber());
            case STRING:
                return new kasperstudios.kashub.server.script.objects.ScriptValue(result.toString());
            case BOOLEAN:
                return new kasperstudios.kashub.server.script.objects.ScriptValue(result.toBoolean());
            case NULL:
            default:
                return kasperstudios.kashub.server.script.objects.ScriptValue.NULL;
        }
    }
